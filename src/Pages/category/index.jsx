...
componentWillMount(){
  ...
  submitCategory = async()=>{
    const lastId = this.state.category.parentId
    ...
    //  如何获取用户输入内容成功向后台发送数据并添加
      ...
      //  添加用户成功再把数据写入到本地
        if(parentId === '0'){
          ...
          console.log(111)
          this.setState({
            twoCategoryData:[...this.state.twoCategoryData,result.data],
            visible:false
          })
          message.success('添加成功')
          resetFields()
  ...
  render(){
    ...
    const {serverData,visible,updateShow,category,twoCategoryData,showDefaultState} = this.state
    const showData = showDefaultState === '0'? serverData:twoCategoryData
      ...
      return (
        <div className="content">
          ...
          >
            ...
            >
              <AddCategoryForm serverData={serverData} showName= {showName} setCategory={form => this.form = form}/>
            </Modal>
          </Card>
        </div>
      )
